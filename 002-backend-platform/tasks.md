...
- [x] T009 [P] [US1] `organizations`, `workspaces`, `memberships` models + migrations (data-model.md §1)
- [x] T010 [P] [US1] Reference data: competitors, categories, regions tables (seeded + wizard create)
- [x] T011 [US1] Setup-wizard endpoints backing `/workspace-setup`
- [ ] T012 [US1] Team invite flow: `memberships` with `status` (active/invited/suspended), invite-by-email
- [x] T013 [P] [US1] Isolation contract tests (confidence/isolation unit gates; full RLS role split still prod)

## Phase 3: M2 — Prompt Library → User Story 2 (P1)

- [x] T014 [P] [US2] `prompts` model + migration, including `samples_per_run`
- [x] T015 [US2] Prompt CRUD + run endpoint backing `/prompts`
- [x] T016 [US2] Quota stub via `PROMPT_QUOTA_DEFAULT` (Stripe replace in M7)
- [ ] T017 [P] [US2] Prompt templates + schedule model

## Phase 4–5: M3–M4 Scan + Analysis (sync mock path)

- [x] Provider protocol + MockProvider (live OpenAI/Anthropic/Google/Perplexity adapters pending T008/T021)
- [x] Deterministic parse + rollup into `metric_daily` with `sample_size` / `parser_version`
- [x] Dashboard / monitoring / answers APIs
- [x] Sync scans (`SYNC_SCANS=true`) for local/dev without Celery
- [ ] Celery beat Redis leader lock + S3 immutable capture (prod path)
- [ ] Formatter migration across all ~15 FE components (partial: KPI cards live)

## Phase 6–8: M5–M7 slices started

- [x] Competitors overview API
- [x] Recommendations list/create/generate/patch (hybrid)
- [x] Experiments CRUD + observational lift + confidence payload
- [x] Billing `/usage` stub (no Stripe)
- [ ] Stripe webhooks, async report S3 downloads, full audit log
